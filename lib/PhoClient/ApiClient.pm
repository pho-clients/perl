...
sub _new_instance
{
    ...
    my (%args) = (
        ...
        'base_url' => 'https://virtserver.swaggerhub.com/phonetworks/server-rest/1.1.0',
        @_
    );
...
sub _cfg {'PhoClient::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $PhoClient::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $PhoClient::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("PhoClient::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $PhoClient::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $PhoClient::Configuration::api_key_prefix->{$key_name};
